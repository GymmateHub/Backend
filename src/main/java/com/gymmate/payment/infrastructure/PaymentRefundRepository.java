...
public interface PaymentRefundRepository extends JpaRepository<PaymentRefund, UUID> {
    ...
    /**
     * @deprecated Use findByOrganisationIdOrderByCreatedAtDesc instead
     */
    @Deprecated(since = "1.0", forRemoval = true)
    List<PaymentRefund> findByGymIdOrderByCreatedAtDesc(UUID gymId);

    /**
     * @deprecated Use findByOrganisationIdAndStatusOrderByCreatedAtDesc instead
     */
    @Deprecated(since = "1.0", forRemoval = true)
    ...
    /**
     * @deprecated Use findByOrganisationIdAndDateRange instead
     */
    @Deprecated(since = "1.0", forRemoval = true)
    ...
    /**
     * @deprecated Use sumRefundAmountByOrganisationIdAndDateRange instead
     */
    @Deprecated(since = "1.0", forRemoval = true)
    @Query("SELECT COALESCE(SUM(r.amount), 0) FROM PaymentRefund r " +
           "WHERE r.gymId = :gymId " +
           "AND r.status = 'SUCCEEDED' " +
           "AND r.createdAt BETWEEN :startDate AND :endDate")
    BigDecimal sumRefundAmountByGymIdAndDateRange(
            @Param("gymId") UUID gymId,
            @Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);
}
