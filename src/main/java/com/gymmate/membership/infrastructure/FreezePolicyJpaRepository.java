...
public interface FreezePolicyJpaRepository extends JpaRepository<FreezePolicy, UUID> {
  ...
  @Query("SELECT fp FROM FreezePolicy fp WHERE fp.gymId = :gymId AND fp.isActive = true")
  Optional<FreezePolicy> findActiveByGymId(@Param("gymId") UUID gymId);

  @Query("SELECT fp FROM FreezePolicy fp WHERE fp.isDefaultPolicy = true AND fp.isActive = true")
  Optional<FreezePolicy> findDefaultPolicy();

  @Query("SELECT fp FROM FreezePolicy fp WHERE fp.organisationId = :organisationId AND fp.isDefaultPolicy = true AND fp.isActive = true")
