...
public interface ClassScheduleJpaRepository extends JpaRepository<ClassSchedule, UUID> {
  ...
  @Query("SELECT cs FROM ClassSchedule cs WHERE cs.gymId = :gymId AND cs.startTime >= :start AND cs.endTime <= :end ORDER BY cs.startTime")
  ...
  @Query("SELECT cs FROM ClassSchedule cs WHERE cs.gymId = :gymId AND cs.startTime >= :start AND cs.endTime <= :end AND cs.status = 'SCHEDULED' ORDER BY cs.startTime")
  ...
  @Query("SELECT cs FROM ClassSchedule cs WHERE cs.gymId = :gymId AND cs.status = :status ORDER BY cs.startTime")
